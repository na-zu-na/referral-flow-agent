...
已提交的免费 scripted 证据位于 `results/d4_scripted_v1/`、`results/d4_scripted_v2/` 和 `results/d4_policy_model_summary.csv`。两种 policy 均运行 40 cases × 3 trials；结构化自动评分和经 `OpenAI Codex (GPT-5)` 复核的最终评分均为 120/120，6 个 negative cases 的 18 次 trials 均通过。Scripted policy 对比用于复现和回归，不代表 live 模型准确率。
