...
const claudeStress = computed<Row | undefined>(() => d6Models.value.find((row) => row.scope === 'FRONTIER_NEGATIVE_ONLY'))
      ...
      <el-tabs v-model="tab">
        ...
        <el-tab-pane label="D6 · Cost analysis" name="d6">
          ...
          <template v-else>
            ...
            <section v-if="claudeStress" class="claude-stress">
              <div class="policy-table-heading"><div><h4>Claude negative-only stress test</h4><p>{{ claudeStress.model }} · not comparable with the four full-battery models.</p></div><el-tag type="warning" effect="plain">Negative only</el-tag></div>
              <div class="distribution-strip"><div><span>Runs</span><strong>{{ claudeStress.runs }}</strong></div><div><span>Negative-case pass</span><strong>{{ formatPercent(claudeStress.negative_subset_pass_rate) }}</strong></div><div><span>Provider spend</span><strong>{{ formatCost(claudeStress.provider_spend_usd) }}</strong></div><div><span>AI cost / run</span><strong>{{ formatCost(claudeStress.avg_provider_ai_cost_measured) }}</strong></div></div>
              <p class="claude-stress-note">This stress test has no overall pass rate or general monthly cost projection.</p>
            </section>
